...
class CustomerRegistration(TransactionBase):
	def get_feed(self):
	...
	def get_customer_name(self):
		...
		if frappe.db.get_value("Customer Registration", self.customer_name) and not frappe.flags.in_import:
			count = frappe.db.sql(
				"""select ifnull(MAX(CAST(SUBSTRING_INDEX(name, ' ', -1) AS UNSIGNED)), 0) from tabCustomer Registration
				 where name like %s""",
	...
	def validate(self):
		...
		if frappe.db.exists("Customer Registration", self.name):
			customer = frappe.get_doc("Customer Registration", self.name)
	...
	def check_customer_group_change(self):
		...
		if not self.get("__islocal"):
			if self.customer_group != frappe.db.get_value("Customer Registration", self.name, "customer_group"):
	...
	def update_customer_groups(self):
		...
		if frappe.flags.customer_group_changed:
			update_linked_doctypes(
				"Customer Registration", self.name, "Customer Group", self.customer_group, ignore_doctypes
			)
	...
	def link_lead_address_and_contact(self):
		if self.lead_name:
			...
			for row in linked_contacts_and_addresses:
				...
				if not linked_doc.has_link("Customer Registration", self.name):
					linked_doc.append("links", dict(link_doctype="Customer Registration", link_name=self.name))
	...
	def on_trash(self):
		...
		delete_contact_and_address("Customer Registration", self.name)
		if self.lead_name:
...
def make_quotation(source_name, target_doc=None):
	...
	target_doc = get_mapped_doc(
		"Customer Registration",
		source_name,
		{"Customer Registration": {"doctype": "Quotation", "field_map": {"name": "party_name"}}},
		target_doc,
		set_missing_values,
	)

	target_doc.quotation_to = "Customer Registration"
	target_doc.run_method("set_missing_values")
	target_doc.run_method("set_other_charges")
	target_doc.run_method("calculate_taxes_and_totals")

	price_list, currency = frappe.db.get_value(
		"Customer Registration", {"name": source_name}, ["default_price_list", "default_currency"]
	)
	if price_list:
...
def get_customer_list(doctype, txt, searchfield, start, page_len, filters=None):
	...
	fields = get_fields("Customer Registration", fields)
...
def get_customer_outstanding(
	...
	outstanding_based_on_gle = frappe.db.sql(
		...
		from `tabGL Entry` where party_type = 'Customer Registration'
...
def get_credit_limit(customer, company):
	...
	if customer:
		credit_limit = frappe.db.get_value(
			"Customer Credit Limit",
			{"parent": customer, "parenttype": "Customer Registration", "company": company},
			"credit_limit",
		)

		if not credit_limit:
			customer_group = frappe.get_cached_value("Customer Registration", customer, "customer_group")
